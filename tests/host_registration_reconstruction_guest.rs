...
fn full_controller_virtio_blk_restore_reconstructs_fresh_host_registrations() {
    match run_full_controller_virtio_blk_host_registration_reconstruction_guest() {
        Ok(result) => {
            ...
            assert_eq!(
                (result.captured_avail_idx(), result.captured_used_idx()),
                (0, 0)
            );
            ...
            assert_eq!(
                mc.page_exact(FULL_CONTROLLER_VIRTIO_BLK_CHECKPOINT_PAGE),
                Some(false)
            );
            ...
            assert_eq!(
                rc.page_exact(FULL_CONTROLLER_VIRTIO_BLK_CHECKPOINT_PAGE),
                Some(true)
            );
            ...
            assert_eq!(
                (result.replay_avail_idx(), result.replay_used_idx()),
                (1, 1)
            );
