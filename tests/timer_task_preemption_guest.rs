...
use mini_hypervisor::task::{
    run_timer_task_preemption_guest, TASK_A_INITIAL_RSP, TASK_A_R12, TASK_A_SAVED_RIP,
    TASK_B_INITIAL_RSP, TASK_B_SAVED_R12, TASK_B_SAVED_RIP, TASK_CONTEXT_PAGE_ADDR,
    TASK_PREEMPTION_GSI, TASK_PREEMPTION_PROOF, TASK_PREEMPTION_TIMER_VECTOR,
    TASK_TERMINAL_USER_RIP,
};
...
#[test]
fn host_timer_preempts_task_a_and_guest_scheduler_restores_it_after_task_b() {
    match run_timer_task_preemption_guest(VmConfig::default()) {
        Ok(result) => {
            assert_eq!(result.gsi(), TASK_PREEMPTION_GSI);
            assert_eq!(result.vector(), TASK_PREEMPTION_TIMER_VECTOR);
            assert_eq!(
            ...
            assert_eq!(result.proof(), TASK_PREEMPTION_PROOF);
            assert_eq!(result.io_exits().len(), TASK_PREEMPTION_PROOF.len());
            for (io, expected) in result
                .io_exits()
                .iter()
                .zip(TASK_PREEMPTION_PROOF.iter().copied())
            {
        ...
        | Err(Error::HostEnvironment(HostEnvironmentError::PermissionDenied { .. })) => {
            eprintln!(
                "skipping timer task preemption integration assertion: /dev/kvm is unavailable to this runner"
            );
        }
        Err(error) => panic!("timer task preemption guest execution failed unexpectedly: {error}"),
    }
}
