...
- require both vCPUs to be stopped at deterministic HLT checkpoint boundaries before capture;
- compose the existing canonical two-vCPU page/register/special-register/MSR ownership with master PIC, slave PIC, IOAPIC, and one LAPIC snapshot for each owned vCPU;
...
- deliberately corrupt all three owned pages, both vCPU states, master PIC, slave PIC, IOAPIC, primary LAPIC, and secondary LAPIC, and require every component to independently mismatch;
- restore bounded page/vCPU state first and require it to verify exactly before mutating controller state;
- then restore master PIC, slave PIC, IOAPIC, primary LAPIC, and secondary LAPIC, followed by exact comparison of the complete ownership set;
- after exact restore, both vCPUs must resume from their original checkpoint boundaries and independently emit proofs `0` and `1` before reaching the existing deterministic terminal HLT RIPs;
