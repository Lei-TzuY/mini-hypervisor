...
- require both vCPUs to be promoted to exact RUNNABLE MP state and stopped at deterministic userspace I/O checkpoint barriers before capture;
...
- after exact restore, both RUNNABLE vCPUs must resume from their userspace checkpoint barriers and independently emit proofs `0` and `1` at deterministic completion barriers without relying on INIT/SIPI or HLT wakeups;
