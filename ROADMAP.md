...
PR #129 seals the coordinated two-VCPU checkpoint frontier at `72fc26574a6f6d90e37f41689b5e8d10511ff3d6`. It captures one canonical shared page set plus exactly two VCPU snapshots only after both deterministic quiescent boundaries, proves independent page/VCPU divergence, restores in canonical order and resumes both VCPUs with exact real-KVM proofs. PRs #130 and #131 add no guest-visible capability; they move the cold builds identified in those audits outside unchanged strict 30-second KVM execution budgets so compilation cannot masquerade as an execution timeout. The final PR #132 validation exposed one remaining workflow with the same defect — `two-vcpu-ap-local-timer` still used `timeout 30s cargo run ...` on a clean runner. This branch corrects that workflow by building first and running the exact binary under the unchanged 30-second timeout, without changing its guest or proof assertions.
