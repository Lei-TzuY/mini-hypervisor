...
impl BoundedTwoVcpuFullControllerTwoVirtioBlkCheckpoint {
    ...
    fn capture_with_pending_notification(
        context: TwoVcpuTwoDeviceCaptureContext<'_>,
        pending_bar: u64,
    ) -> Result<(Self, VirtioBlkPendingNotificationToken), Error> {
        let bars = canonical_two_virtio_blk_bars(context.bars)?;
        if pending_bar != bars[0] && pending_bar != bars[1] {
            return Err(page_set_error(
                "two-vCPU pending-notification checkpoint binding",
                format!("pending BAR {pending_bar:#x} is outside canonical BARs {bars:?}"),
            ));
        }
        ...
        let (first_device, token, second_device) = if pending_bar == bars[0] {
            let (pending, token) = context
                .mmio
                .capture_virtio_blk_checkpoint_with_pending_notification_at(bars[0])?
                .ok_or_else(|| {
                    page_set_error(
                        "two-vCPU pending-notification checkpoint capture",
                        format!("pending virtio-blk device at BAR {:#x} is missing", bars[0]),
                    )
                })?;
            let other = capture_required_device(
            ...
            let (pending, token) = context
                .mmio
                .capture_virtio_blk_checkpoint_with_pending_notification_at(bars[1])?
                .ok_or_else(|| {
                    page_set_error(
                        "two-vCPU pending-notification checkpoint capture",
                        format!("pending virtio-blk device at BAR {:#x} is missing", bars[1]),
    ...
    pub(crate) fn restore_and_verify_with_pending_notification(
        &self,
        first: &mut Vcpu,
        second: &mut Vcpu,
        vm: &mut crate::kvm::Vm,
        ...
        mmio.restore_two_virtio_blk_checkpoints_atomic_with_pending_notification(
            [
                (self.devices[0].0, &self.devices[0].1),
                (self.devices[1].0, &self.devices[1].1),
            ],
...
impl TwoVcpuTwoDeviceCheckpointTransaction {
    ...
    fn capture_with_pending_notification(
        ...
        registrations.require_checkpoint_quiescent()?;
        let (checkpoint, token) =
            BoundedTwoVcpuFullControllerTwoVirtioBlkCheckpoint::capture_with_pending_notification(
                context,
                pending_bar,
            )?;
        Ok((
    ...
    fn restore_and_reconstruct_with_pending_notification(
        &self,
        backend: &crate::kvm::KvmBackend,
        first: &mut Vcpu,
        second: &mut Vcpu,
    ...
    > {
        let restored = self
            .checkpoint
            .restore_and_verify_with_pending_notification(first, second, vm, mmio, token)?;
        if !restored.is_exact_match() {
            return Err(page_set_error(
                "two-vCPU pending-notification transaction restore",
        ...
        if let Err(error) = registrations.require_checkpoint_quiescent() {
            ...
            return match cleanup {
                Ok(()) => Err(error),
                Err(cleanup_error) => Err(page_set_error(
                    "two-vCPU pending-notification reconstruction cleanup",
                    format!(
                        "reconstructed pair was not quiescent: {error}; cleanup also failed: {cleanup_error}"
                    ),
                )),
        ...
        let post_reconstruction = match self.checkpoint.verify(first, second, vm, mmio) {
            ...
            Err(error) => {
                ...
                return match cleanup {
                    Ok(()) => Err(error),
                    Err(cleanup_error) => Err(page_set_error(
                        "two-vCPU pending-notification post-registration cleanup",
                        format!(
                            "post-registration verification failed: {error}; cleanup also failed: {cleanup_error}"
                        ),
                    )),
        ...
        if !post_reconstruction.is_exact_match() {
            ...
            return match cleanup {
                Ok(()) => Err(page_set_error(
                    "two-vCPU pending-notification post-registration verification",
        ...
        token
            .validate_device(self.checkpoint.device(token.bar0()).ok_or_else(|| {
                page_set_error(
                    "two-vCPU pending-notification restored token binding",
                    format!("token BAR {:#x} disappeared", token.bar0()),
                )
            })?)
            .map_err(|error| {
                page_set_error(
                    "two-vCPU pending-notification restored token validation",
                    error.to_string(),
                )
            })?;
