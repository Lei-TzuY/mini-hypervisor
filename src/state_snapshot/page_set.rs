...
use crate::long_mode::{LONG_MODE_PAGE_SIZE, LONG_MODE_PAGE_SIZE_USIZE};
...
mod tests {
    ...
    #[test]
    fn page_comparison_lookup_uses_canonical_addresses() {
        let comparison = BoundedPageSetCheckpointComparison {
            pages: vec![
            ...
            vcpu: VcpuStateSnapshotComparison {
                registers: unreachable_comparison(),
                special_registers: unreachable_special_comparison(),
                msrs: unreachable_msr_comparison(),
            },
        };
        assert_eq!(comparison.page_exact(GuestPhysAddr::new(0x22000)), Some(true));
        assert_eq!(comparison.page_exact(GuestPhysAddr::new(0x30000)), Some(false));
        assert_eq!(comparison.page_exact(GuestPhysAddr::new(0x40000)), None);
    }

    fn unreachable_comparison() -> crate::vcpu::VcpuRegisterSnapshotComparison {
        panic!("comparison fixture must not construct VCPU components")
    }

    fn unreachable_special_comparison() -> crate::vcpu::VcpuSpecialRegisterSnapshotComparison {
        panic!("comparison fixture must not construct VCPU components")
    }

    fn unreachable_msr_comparison() -> crate::kvm::msr::value_set::GuestMsrSnapshotComparison {
        panic!("comparison fixture must not construct VCPU components")
    }
}
