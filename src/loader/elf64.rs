...
const PROOF_SEGMENT_ADDRESS: u64 = 0x1_0000;
const PROOF_CODE_OFFSET: usize = 0x100;
const PROOF_ENTRY: u64 = PROOF_SEGMENT_ADDRESS + PROOF_CODE_OFFSET as u64;
...
impl<'a> Elf64GuestImage<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, Elf64Error> {
        ...
        let mut segments: Vec<Elf64LoadSegment> = Vec::new();
...
pub fn run_elf64_guest(config: VmConfig) -> Result<Elf64GuestResult, Error> {
    ...
    let image = Elf64GuestImage::parse(&bytes)
        .expect("the built-in deterministic ELF64 proof fixture remains structurally valid");
    let backend = KvmBackend::open()?;
    let mut vm = backend.create_vm()?;
    let mut memory = GuestMemory::new(GuestPhysAddr::new(0), LONG_MODE_IDENTITY_MAP_SIZE)?;
    let layout = LongModeBootLayout::new(memory.region(), image.entry(), PROOF_STACK_POINTER)
        .expect("the validated ELF64 proof entry remains inside the fixed long-mode layout");
...
fn proof_fixture() -> Vec<u8> {
    let file_size = PROOF_CODE_OFFSET + PROOF_CODE.len();
    ...
    write_u64(&mut bytes, 24, PROOF_ENTRY);
    ...
    write_u64(&mut bytes, ph + 16, PROOF_SEGMENT_ADDRESS);
    write_u64(&mut bytes, ph + 24, PROOF_SEGMENT_ADDRESS);
    write_u64(&mut bytes, ph + 32, file_size as u64);
    write_u64(&mut bytes, ph + 40, PROOF_MEMORY_SIZE);
    write_u64(&mut bytes, ph + 48, 0x1000);
    bytes[PROOF_CODE_OFFSET..file_size].copy_from_slice(&PROOF_CODE);
    bytes
}
...
mod tests {
    ...
    fn fixture() -> Vec<u8> {
        let file_size = PROOF_CODE_OFFSET + TEST_CODE.len();
        let mut bytes = vec![0_u8; file_size];
        bytes[0..4].copy_from_slice(&[0x7f, b'E', b'L', b'F']);
        bytes[4] = ELFCLASS64;
        bytes[5] = ELFDATA2LSB;
        ...
        write_u64(&mut bytes, ph + 40, PROOF_MEMORY_SIZE);
        write_u64(&mut bytes, ph + 48, 0x1000);
        bytes[PROOF_CODE_OFFSET..file_size].copy_from_slice(&TEST_CODE);
        bytes
    }
    ...
    fn parses_bounded_identity_mapped_x86_64_executable() {
        ...
        assert_eq!(image.entry().get(), PROOF_ENTRY);
        assert_eq!(image.segments().len(), 1);
        let segment = image.segments()[0];
        assert_eq!(segment.program_header_index(), 0);
        assert_eq!(segment.guest_address().get(), PROOF_SEGMENT_ADDRESS);
        assert_eq!(segment.file_size(), bytes.len());
        assert_eq!(segment.memory_size(), PROOF_MEMORY_SIZE as usize);
    ...
    fn load_copies_file_bytes_and_explicitly_zeroes_bss() {
        ...
        memory
            .read(
                GuestPhysAddr::new(PROOF_SEGMENT_ADDRESS + bytes.len() as u64),
                &mut observed_bss,
            )
            .unwrap();
    ...
    fn rejects_non_identity_mapped_or_bootstrap_overlapping_segments() {
        let ph = ELF64_HEADER_SIZE;
        let mut bytes = fixture();
        write_u64(&mut bytes, ph + 24, PROOF_SEGMENT_ADDRESS + 0x1000);
        assert!(matches!(
            Elf64GuestImage::parse(&bytes),
    ...
    fn rejects_entry_outside_executable_file_backed_range() {
        let mut bytes = fixture();
        let entry = PROOF_SEGMENT_ADDRESS + PROOF_MEMORY_SIZE - 1;
    ...
    #[test]
    fn deterministic_fixture_parses_with_expected_entry_and_terminal_rip() {
        let bytes = proof_fixture();
        let image = Elf64GuestImage::parse(&bytes).unwrap();
        assert_eq!(image.entry().get(), PROOF_ENTRY);
        assert_eq!(proof_terminal_rip(), PROOF_ENTRY + PROOF_CODE.len() as u64);
