...
const AP_TRAMPOLINE_BYTES: [u8; 27] = [
    ...
    0xb0, SHARED_MARKER_VALUE, // marker value
    0xa2, 0x00, 0x90, // mov [0x9000], al using 16-bit moffs
    0xb0, b'P', 0xe6, 0xe9, // shared-memory write completed
...
pub fn run_two_vcpu_init_sipi() -> Result<TwoVcpuInitSipiResult, Error> {
    ...
    let trampoline = FlatGuestImage::new(
        AP_TRAMPOLINE,
        AP_TRAMPOLINE,
        &AP_TRAMPOLINE_BYTES,
    )?;
