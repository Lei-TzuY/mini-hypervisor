...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct ApStartupObservation { mp_state: u32, rip: u64, cs_selector: u16, cs_base: u64 }
...
struct ApWorkerResult {
    io_exits: Vec<PortIoExit>, proof: Vec<u8>, ready_rflags: u64, completion_rflags: u64,
    startup: ApStartupObservation, rsp: u64, cs_selector: u16, cs_long: u8, ss_selector: u16,
    gdt_base: u64, gdt_limit: u16, idt_base: u64, idt_limit: u16, cr0: u64, cr3: u64, cr4: u64, efer: u64,
}
...
pub fn run_two_vcpu_tlb_shootdown() -> Result<TwoVcpuTlbShootdownResult, Error> {
    ...
    let bsp_layout = LongModeMmioBootLayout::with_device_mappings(
        memory.region(), bsp_image.entry(), BSP_STACK,
    ...
    let ap_interrupt_layout = LongModeInterruptLayout::new(
        memory.region(), AP_TRAMPOLINE, AP_LONG_MODE_STACK,
        TLB_SHOOTDOWN_VECTOR, TLB_SHOOTDOWN_HANDLER,
    ...
    {
        return Err(verification_error(SECOND_VCPU_ID.get(), "TLB-shootdown AP IDT layout",
    ...
    bsp_image.load(&mut memory)?; ap_image.load(&mut memory)?; handler.load(&mut memory)?;
    ...
    if initial_ap_mp_state != KVM_MP_STATE_UNINITIALIZED {
        return Err(verification_error(SECOND_VCPU_ID.get(), "initial AP MP state",
    ...
    let worker = std::thread::spawn(move || -> Result<ApWorkerResult, Error> {
        ...
        if idt.base()!=LONG_MODE_INTERRUPT_IDT_ADDR.get() || idt.limit()!=TLB_SHOOTDOWN_IDT_LIMIT {
            return Err(verification_error(SECOND_VCPU_ID.get(),"AP TLB ready IDT",
        ...
        let special=ap_vcpu.capture_special_register_snapshot()?;
        let cs=special.cs(); let ss=special.ss(); let gdt=special.gdt();
        let proof=port_io.debug_output().unwrap_or(&[]).to_vec();
        if proof.as_slice()!=AP_TLB_PROOF { return Err(verification_error(SECOND_VCPU_ID.get(),"AP TLB proof",format!("expected {AP_TLB_PROOF:?}, got {proof:?}"))); }
        Ok(ApWorkerResult{io_exits,proof,ready_rflags,completion_rflags,startup,rsp:regs.rsp(),cs_selector:cs.selector(),cs_long:cs.l(),ss_selector:ss.selector(),gdt_base:gdt.base(),gdt_limit:gdt.limit(),idt_base:special.idt().base(),idt_limit:special.idt().limit(),cr0:special.cr0(),cr3:special.cr3(),cr4:special.cr4(),efer:special.efer()})
    });
    ...
    for (byte,stage) in [(b'P',"shared PTE mutation"),(b'X',"shootdown IPI")] {
        match run_expected_debug_output(&mut bsp_vcpu,&mut bsp_port_io,byte,stage) {
            Ok(exit)=>bsp_io_exits.push(exit), Err(error)=>{let _=command_tx.send(ApWorkerCommand::Abort);let _=worker.join();return Err(error);}
        }
    }
    ...
    if bsp_proof.as_slice()!=BSP_TLB_PROOF { return Err(verification_error(FIRST_VCPU_ID.get(),"BSP TLB proof",format!("expected {BSP_TLB_PROOF:?}, got {bsp_proof:?}"))); }
    ...
    let mut pte_bytes=[0_u8;8]; guest_memory.read(TLB_TARGET_PTE,&mut pte_bytes)?; let final_pte=u64::from_le_bytes(pte_bytes);
    let mut ack=[0_u8;1]; let mut page_a=[0_u8;1]; let mut page_b=[0_u8;1];
    guest_memory.read(SHOOTDOWN_ACK,&mut ack)?; guest_memory.read(TLB_PAGE_A,&mut page_a)?; guest_memory.read(TLB_PAGE_B,&mut page_b)?;
    if final_pte!=TLB_FINAL_PTE || ack[0]!=0 || page_a[0]!=TLB_PAGE_A_VALUE || page_b[0]!=TLB_PAGE_B_VALUE {
    ...
    Ok(TwoVcpuTlbShootdownResult{bsp_io_exits,ap_io_exits:ap.io_exits,bsp_proof,ap_proof:ap.proof,state:TlbShootdownState{initial_ap_mp_state,startup_rip:ap.startup.rip,startup_cs_selector:ap.startup.cs_selector,startup_cs_base:ap.startup.cs_base,ready_rflags:ap.ready_rflags,completion_rflags:ap.completion_rflags,rsp:ap.rsp,cs_selector:ap.cs_selector,cs_long:ap.cs_long,ss_selector:ap.ss_selector,gdt_base:ap.gdt_base,gdt_limit:ap.gdt_limit,idt_base:ap.idt_base,idt_limit:ap.idt_limit,cr0:ap.cr0,cr3:ap.cr3,cr4:ap.cr4,efer:ap.efer},final_pte,final_ack:ack[0],page_a:page_a[0],page_b:page_b[0]})
}

fn require_init_sipi_startup_state(vcpu:&mut Vcpu)->Result<ApStartupObservation,Error>{
 let mp_state=vcpu.accept_init_sipi_startup_handoff()?; let registers=vcpu.registers()?; let special=vcpu.capture_special_register_snapshot()?; let cs=special.cs();
 if mp_state!=KVM_MP_STATE_RUNNABLE || registers.rip!=0 || cs.selector()!=SIPI_CS_SELECTOR || cs.base()!=SIPI_CS_BASE || special.cr0()&X86_CR0_PROTECTED_MODE_ENABLE!=0 {return Err(verification_error(vcpu.id().get(),"AP SIPI startup",format!("mp={mp_state} rip={:#x} cs={:#x} base={:#x} cr0={:#x}",registers.rip,cs.selector(),cs.base(),special.cr0())));}
...
fn validate_ap_state(ap:&ApWorkerResult)->Result<(),Error>{
 let valid=ap.startup.mp_state==KVM_MP_STATE_RUNNABLE&&ap.rsp==AP_LONG_MODE_STACK&&ap.cs_selector==AP_LONG_MODE_CODE_SELECTOR&&ap.cs_long==1&&ap.ss_selector==AP_LONG_MODE_DATA_SELECTOR&&ap.gdt_base==AP_LONG_MODE_GDT.get()&&ap.gdt_limit==AP_LONG_MODE_GDT_LIMIT&&ap.idt_base==LONG_MODE_INTERRUPT_IDT_ADDR.get()&&ap.idt_limit==TLB_SHOOTDOWN_IDT_LIMIT&&ap.cr0&LONG_MODE_CR0_REQUIRED_BITS==LONG_MODE_CR0_REQUIRED_BITS&&ap.cr4&LONG_MODE_CR4_REQUIRED_BITS==LONG_MODE_CR4_REQUIRED_BITS&&ap.efer&LONG_MODE_EFER_REQUIRED_BITS==LONG_MODE_EFER_REQUIRED_BITS&&ap.cr3==LONG_MODE_PML4_ADDR.get();
 if !valid{return Err(verification_error(SECOND_VCPU_ID.get(),"AP post-shootdown long-mode state","AP architectural state drifted from integrated SIPI/long-mode contract"));} Ok(())
}
fn run_expected_debug_output(vcpu:&mut Vcpu,port_io:&mut PortIoBus,expected:u8,stage:&'static str)->Result<PortIoExit,Error>{let exit=vcpu.run_once()?;if exit!=VcpuExit::Io{return Err(verification_error(vcpu.id().get(),stage,format!("expected KVM_EXIT_IO, got {exit:?}")));}let io=vcpu.port_io_exit()?;if io.direction()!=PortIoDirection::Out||io.port()!=DEBUG_PORT||io.size()!=1||io.count()!=1||io.output_data()!=[expected]{return Err(verification_error(vcpu.id().get(),stage,format!("unexpected debug exit {io:?}; expected {expected:#x}")));}if port_io.dispatch(&io)?!=PortIoService::Output{return Err(verification_error(vcpu.id().get(),stage,"debug output requested input response"));}Ok(io)}
fn require_interrupt_disabled_flags(id:u16,stage:&'static str,rflags:u64)->Result<(),Error>{if rflags&X86_RFLAGS_RESERVED_BIT!=X86_RFLAGS_RESERVED_BIT||rflags&X86_RFLAGS_INTERRUPT_ENABLE!=0{return Err(verification_error(id,stage,format!("expected bit1 with IF clear, got {rflags:#x}")));}Ok(())}
fn require_interrupt_enabled_flags(id:u16,stage:&'static str,rflags:u64)->Result<(),Error>{if rflags&X86_RFLAGS_RESERVED_BIT!=X86_RFLAGS_RESERVED_BIT||rflags&X86_RFLAGS_INTERRUPT_ENABLE!=X86_RFLAGS_INTERRUPT_ENABLE{return Err(verification_error(id,stage,format!("expected bit1+IF, got {rflags:#x}")));}Ok(())}
fn write_u64(memory:&mut GuestMemory,address:GuestPhysAddr,value:u64)->Result<(),Error>{memory.write(address,&value.to_le_bytes())}
fn verification_error(id:u16,operation:&'static str,detail:impl Into<String>)->Error{Error::HostEnvironment(HostEnvironmentError::VcpuOperation{id,operation,source:io::Error::new(io::ErrorKind::InvalidData,detail.into())})}
...
mod tests {
    ...
    fn target_alias_uses_shared_page_table_entry_outside_lapic_slot() {
        ...
        assert_eq!(TLB_PAGE_B.get() | PAGE_TABLE_ENTRY_FLAGS | PAGE_TABLE_ENTRY_ACCESSED, TLB_FINAL_PTE);
    ...
    fn ap_preserves_integrated_sipi_transition_and_executes_invlpg_before_ack() {
        ...
        assert!(AP_HANDLER_BYTES.windows(4).any(|w| w == [0xb0,b'I',0xe6,0xe9]));
    }

    #[test]
    fn proofs_and_idt_contract_are_fixed() {
        assert_eq!(BSP_TLB_PROOF,b"0IDSPXAD"); assert_eq!(AP_TLB_PROOF,b"ALRIBD");
        assert_eq!(TLB_SHOOTDOWN_VECTOR,0x54); assert_eq!(TLB_SHOOTDOWN_IDT_LIMIT,0x054f);
