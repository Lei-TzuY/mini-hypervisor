...
pub const TASK_WAKE_GSI: u32 = 0;
pub const TASK_WAKE_TIMER_VECTOR: u8 = 0x40;
pub const TASK_BLOCK_VECTOR: u8 = 0x7e;
pub const TASK_WAKE_ARM_VECTOR: u8 = 0x7f;
pub const TASK_BLOCK_HANDLER: GuestPhysAddr = GuestPhysAddr::new(0x14000);
pub const TASK_WAKE_ARM_HANDLER: GuestPhysAddr = GuestPhysAddr::new(0x16000);
pub const TASK_WAKE_TIMER_HANDLER: GuestPhysAddr = GuestPhysAddr::new(0x17000);
pub const TASK_RUN_STATE_ADDR: GuestPhysAddr = GuestPhysAddr::new(0x300c0);
pub const TASK_BLOCK_WAKE_PROOF: &[u8; 7] = b"KAPWBRD";

const TASK_WAKE_TIMER_DELAY_MILLIS: u64 = 10;
const TASK_WAKE_WATCHDOG_SECONDS: u64 = 5;
const TASK_BLOCK_BYTE: u8 = b'K';
const TASK_WAKE_ARM_BYTE: u8 = b'P';
const TASK_WAKE_BYTE: u8 = b'W';
const TASK_WAKE_FAILURE_BYTE: u8 = b'F';
...
pub struct TaskBlockWakeGuestResult {
    gsi: u32,
    vector: u8,
    lapic_spiv: u32,
    lapic_lint0: u32,
    armed_rflags: u64,
    blocked_state: TaskRunState,
    wake_state: TaskRunState,
    final_state: TaskRunState,
...
pub fn run_task_block_wake_guest(config: VmConfig) -> Result<TaskBlockWakeGuestResult, Error> {
    ...
    let layout = AddressSpaceSwitchLayout::new(memory.region())
        .expect("fixed bounded task block/wakeup layout remains valid");
    layout.install_tables(&mut memory)?;
    install_wakeup_gates(&mut memory, &layout)?;
    ...
    memory.write(TASK_RUN_STATE_ADDR, &[TaskRunState::Runnable as u8])?;
    ...
    let timer_irq = vm
        ...
        .map_err(|source| wakeup_vm_error("duplicate task wake IRQ-line handle", source))?;
    let watchdog_irq = vm
        .duplicate_irq_line_handle()
        .map_err(|source| wakeup_vm_error("duplicate task wake watchdog handle", source))?;
    timer_irq
        .set_gsi_level(TASK_WAKE_GSI, false)
        .map_err(|source| wakeup_vm_error("preflight task wake IRQ line", source))?;
    watchdog_irq
        .set_gsi_level(TASK_WAKE_GSI, false)
    ...
    let timer_result = timer_worker.join().map_err(|_| {
        verification_error(
            "join task wake timer worker",
            "task wake timer worker panicked before reporting its GSI pulse",
        )
    })?;
    let watchdog_fired = watchdog_worker
        ...
        .map_err(|_| {
            verification_error(
                "join task wake watchdog",
                "task wake watchdog panicked before reporting whether it fired",
            )
        })?
        .map_err(|source| wakeup_vm_error("task wake watchdog GSI", source))?;
    timer_result.map_err(|source| wakeup_vm_error("task wake GSI pulse", source))?;
    if watchdog_fired {
        return Err(verification_error(
            "task block/wakeup watchdog",
            "watchdog injected fallback GSI; external wakeup was not independently proven",
        ));
    }
    let (wake_io, wake_state, scheduler_b_io, restored_io, done_io) = execution?;
    let io_exits = vec![
        blocked_io,
        scheduler_a_io,
        armed_io,
    ...
    if proof.as_slice() != TASK_BLOCK_WAKE_PROOF || io_exits.len() != TASK_BLOCK_WAKE_PROOF.len() {
        return Err(verification_error(
            "task block/wakeup proof",
            format!(
                "expected {:?} across {} exits, got {:?} across {} exits",
                TASK_BLOCK_WAKE_PROOF,
                TASK_BLOCK_WAKE_PROOF.len(),
                proof,
                io_exits.len()
            ),
        ));
...
fn encode_wakeup_interrupt_gate(handler: u64, user_callable: bool) -> [u8; 16] {
...
fn read_task_run_state(memory: &GuestMemory) -> Result<TaskRunState, Error> {
    match read_byte(memory, TASK_RUN_STATE_ADDR)? {
        0 => Ok(TaskRunState::Blocked),
        1 => Ok(TaskRunState::Runnable),
        value => Err(verification_error(
            "task run-state encoding",
...
fn require_task_state(stage: &'static str, actual: TaskRunState, expected: TaskRunState) -> Result<(), Error> {
    if actual != expected {
        return Err(verification_error(
            stage,
            format!("expected task state {expected:?}, got {actual:?}"),
        ));
    }
    Ok(())
}

fn require_wakeup_interrupt_disabled_flags(stage: &'static str, rflags: u64) -> Result<(), Error> {
...
fn validate_wakeup_task_state(
    ...
    if task_a != expected_a || task_b != expected_b || terminal != expected_terminal {
        return Err(verification_error(
            "task block/wakeup context ownership",
            format!(
    ...
    if final_cr3 != ADDRESS_SPACE_A_CR3.get() || final_r12 != TASK_A_R12 {
        return Err(verification_error(
            "task block/wakeup final registers",
            format!(
    ...
    if task_a_stack_marker != b'a' || task_b_stack_marker != b'b' {
        return Err(verification_error(
            "task block/wakeup stack ownership",
            format!(
...
fn wakeup_vm_error(operation: &'static str, source: io::Error) -> Error {
    Error::HostEnvironment(HostEnvironmentError::VmOperation { operation, source })
}

const _: () = {
    assert!(TASK_RUN_STATE_ADDR.get() >= TASK_CONTEXT_PAGE_ADDR.get());
    assert!(TASK_RUN_STATE_ADDR.get() < TASK_CONTEXT_PAGE_ADDR.get() + LONG_MODE_PAGE_SIZE);
    assert!(TASK_BLOCK_HANDLER.get() < LONG_MODE_IDENTITY_MAP_SIZE);
    assert!(TASK_WAKE_ARM_HANDLER.get() < LONG_MODE_IDENTITY_MAP_SIZE);
    assert!(TASK_WAKE_TIMER_HANDLER.get() < LONG_MODE_IDENTITY_MAP_SIZE);
};
...
mod tests {
    ...
    fn wakeup_gates_are_installed_for_timer_block_and_arm_vectors() {
        ...
        for (vector, handler, access) in [
            ...
            let mut gate = [0_u8; 16];
            memory.read(address, &mut gate).unwrap();
            let decoded = u64::from(u16::from_le_bytes([gate[0], gate[1]]))
