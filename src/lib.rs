...
pub fn run_hlt_guest(config: VmConfig) -> Result<VmExitReport, Error> {
    ...
    match dispatch_vcpu_exit(&vcpu, exit, &mut port_io)? {
...
pub fn run_debug_port_guest(config: VmConfig) -> Result<DebugPortGuestResult, Error> {
    ...
    let io = match dispatch_vcpu_exit(&vcpu, first_exit, &mut port_io)? {
    ...
    let report = match dispatch_vcpu_exit(&vcpu, second_exit, &mut port_io)? {
