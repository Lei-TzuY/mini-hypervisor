...
pub struct VcpuExceptionExit {
    exception: u32,
    error_code: u32,
}

impl VcpuExceptionExit {
    #[must_use]
...
impl Vcpu {
    pub fn exception_exit(&self) -> Result<VcpuExceptionExit, Error> {
        self.run.exception_exit(self.id)
    }
}
...
impl KvmRunMapping {
    fn exception_exit(&self, id: VcpuId) -> Result<VcpuExceptionExit, Error> {
...
const fn decode_exception(raw: KvmRunException) -> VcpuExceptionExit {
    VcpuExceptionExit {
...
mod tests {
    ...
    fn exception_decoder_copies_vector_and_error_code_exactly() {
        let decoded = decode_exception(KvmRunException {
            exception: 14,
            error_code: 0x1234_5678,
        });

        assert_eq!(decoded.exception(), 14);
        assert_eq!(decoded.error_code(), 0x1234_5678);
    }
}
