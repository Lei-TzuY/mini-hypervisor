use super::virtio::{
    ...
    VIRTIO_NOTIFY_LENGTH, VIRTIO_NOTIFY_OFF_MULTIPLIER, VIRTIO_NOTIFY_OFFSET,
...
impl fmt::Display for VirtioBlkError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnsupportedRegisterAccess { offset, length, write } => write!(
                f,
                "unsupported virtio-blk {} access at BAR offset {offset:#x} with length {length}",
                if *write { "write" } else { "read" }
            ),
            Self::InvalidRegisterPayload { offset, expected, actual } => write!(
                f,
            ...
            Self::InvalidDescriptorFlags { index, expected, actual } => write!(
                f,
                "virtio-blk descriptor {index} flags {actual:#x} do not match required {expected:#x}"
            ),
            Self::DescriptorTooSmall { index, length, required } => write!(
                f,
            ...
            Self::InvalidRequestReserved { reserved } => {
                write!(f, "virtio-blk request reserved field must be zero, got {reserved:#x}")
            }
...
impl VirtioBlkQueueCompletion {
    #[must_use]
    pub const fn descriptor_id(self) -> u32 { self.descriptor_id }
    #[must_use]
    pub const fn length(self) -> u32 { self.length }
    #[must_use]
    pub const fn sector(self) -> u64 { self.sector }
}
...
impl VirtioBlkPciFunction {
    ...
    pub const fn new(bar0: u32) -> Self {
        Self { bar0: bar0 & 0xffff_f000 }
    }

    #[must_use]
    pub const fn bar0(&self) -> u32 { self.bar0 }
...
impl VirtioBlkDevice {
    ...
    #[must_use]
    pub const fn bar0(&self) -> u64 { self.bar0 }
    #[must_use]
    ...
    #[must_use]
    pub const fn isr_status(&self) -> u8 { self.isr_status }
    ...
    pub fn read(&mut self, offset: u64, length: usize) -> Result<Vec<u8>, VirtioBlkError> {
        let bytes = match (offset, length) {
            ...
            _ => return Err(VirtioBlkError::UnsupportedRegisterAccess { offset, length, write: false }),
        };
        Ok(bytes)
    }

    pub fn write(&mut self, offset: u64, payload: &[u8]) -> Result<Option<VirtioBlkEvent>, VirtioBlkError> {
        match (offset, payload.len()) {
            (COMMON_DEVICE_FEATURE_SELECT, 4) => self.device_feature_select = read_u32(offset, payload)?,
            (COMMON_DRIVER_FEATURE_SELECT, 4) => self.driver_feature_select = read_u32(offset, payload)?,
            (COMMON_DRIVER_FEATURE, 4) => self.write_driver_feature_word(read_u32(offset, payload)?)?,
            ...
            (COMMON_QUEUE_DESC, 4) => { self.ensure_queue_unlocked()?; self.queue_desc = replace_low_u32(self.queue_desc, read_u32(offset, payload)?); }
            (COMMON_QUEUE_DESC_HIGH, 4) => { self.ensure_queue_unlocked()?; self.queue_desc = replace_high_u32(self.queue_desc, read_u32(offset, payload)?); }
            (COMMON_QUEUE_DRIVER, 4) => { self.ensure_queue_unlocked()?; self.queue_driver = replace_low_u32(self.queue_driver, read_u32(offset, payload)?); }
            (COMMON_QUEUE_DRIVER_HIGH, 4) => { self.ensure_queue_unlocked()?; self.queue_driver = replace_high_u32(self.queue_driver, read_u32(offset, payload)?); }
            (COMMON_QUEUE_DEVICE, 4) => { self.ensure_queue_unlocked()?; self.queue_device = replace_low_u32(self.queue_device, read_u32(offset, payload)?); }
            (COMMON_QUEUE_DEVICE_HIGH, 4) => { self.ensure_queue_unlocked()?; self.queue_device = replace_high_u32(self.queue_device, read_u32(offset, payload)?); }
            (VIRTIO_NOTIFY_OFFSET, 2) => {
                let queue = read_u16(offset, payload)?;
                if queue != VIRTIO_BLK_QUEUE_INDEX { return Err(VirtioBlkError::UnexpectedQueueNotification { queue }); }
                self.ensure_queue_ready()?;
                self.notify_pending = true;
                return Ok(Some(VirtioBlkEvent::QueueNotified { queue }));
            }
            _ => return Err(VirtioBlkError::UnsupportedRegisterAccess { offset, length: payload.len(), write: true }),
        }
        Ok(None)
    }

    pub fn process_notified_queue(&mut self, memory: &mut GuestMemory) -> Result<VirtioBlkQueueCompletion, VirtioBlkProcessError> {
        self.ensure_queue_ready()?;
        if !self.notify_pending { return Err(VirtioBlkError::QueueNotReady.into()); }

        let avail_idx = read_guest_u16(memory, checked_add(self.queue_driver, 2)?)?;
        let expected_avail = self.last_avail_idx.wrapping_add(1);
        if avail_idx != expected_avail { return Err(VirtioBlkError::UnexpectedAvailIndex { expected: expected_avail, actual: avail_idx }.into()); }
        let slot = self.last_avail_idx % self.queue_size;
        let head = read_guest_u16(memory, checked_add(self.queue_driver, 4 + 2 * u64::from(slot))?)?;
        ...
        if data_index == head { return Err(VirtioBlkError::DescriptorChainCycle { index: data_index }.into()); }

        let data = self.read_descriptor(memory, data_index)?;
        self.require_flags(data_index, data.flags, VIRTQ_DESC_F_NEXT | VIRTQ_DESC_F_WRITE)?;
        ...
        if status_index == head || status_index == data_index { return Err(VirtioBlkError::DescriptorChainCycle { index: status_index }.into()); }
        ...
        if request_type != VIRTIO_BLK_T_IN { return Err(VirtioBlkError::InvalidRequestType { request_type }.into()); }
        if reserved != 0 { return Err(VirtioBlkError::InvalidRequestReserved { reserved }.into()); }
        if sector >= VIRTIO_BLK_CAPACITY_SECTORS { return Err(VirtioBlkError::SectorOutOfRange { sector, capacity: VIRTIO_BLK_CAPACITY_SECTORS }.into()); }

        let used_idx = read_guest_u16(memory, checked_add(self.queue_device, 2)?)?;
        if used_idx != self.last_used_idx { return Err(VirtioBlkError::UnexpectedUsedIndex { expected: self.last_used_idx, actual: used_idx }.into()); }
        ...
        memory.write(GuestPhysAddr::new(checked_add(self.queue_device, 2)?), &next_used.to_le_bytes())?;

        self.last_avail_idx = avail_idx;
        self.last_used_idx = next_used;
        self.notify_pending = false;
        self.isr_status |= VIRTIO_ISR_QUEUE_INTERRUPT;
        Ok(VirtioBlkQueueCompletion { descriptor_id: u32::from(head), length: written, sector })
    }
    ...
    fn ensure_descriptor_index(&self, index: u16) -> Result<(), VirtioBlkError> {
        if index >= self.queue_size { Err(VirtioBlkError::DescriptorIndexOutOfRange { index, queue_size: self.queue_size }) } else { Ok(()) }
    }

    fn require_flags(&self, index: u16, actual: u16, expected: u16) -> Result<(), VirtioBlkError> {
        if actual == expected { Ok(()) } else { Err(VirtioBlkError::InvalidDescriptorFlags { index, expected, actual }) }
    }

    fn require_length(&self, index: u16, length: u32, required: u32) -> Result<(), VirtioBlkError> {
        if length >= required { Ok(()) } else { Err(VirtioBlkError::DescriptorTooSmall { index, length, required }) }
    }

    fn device_feature_word(&self) -> u32 {
        match self.device_feature_select { 1 => (VIRTIO_F_VERSION_1 >> 32) as u32, _ => 0 }
    }

    fn driver_feature_word(&self) -> u32 {
        match self.driver_feature_select { 0 => self.driver_features as u32, 1 => (self.driver_features >> 32) as u32, _ => 0 }
    }

    fn write_driver_feature_word(&mut self, value: u32) -> Result<(), VirtioBlkError> {
        if self.status & VIRTIO_STATUS_FEATURES_OK != 0 { return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested: self.status }); }
        match self.driver_feature_select {
            0 => self.driver_features = (self.driver_features & !0xffff_ffff) | u64::from(value),
            1 => self.driver_features = (self.driver_features & 0xffff_ffff) | (u64::from(value) << 32),
            _ if value == 0 => {}
            _ => return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested: self.status }),
        }
        Ok(())
    }

    fn write_status(&mut self, requested: u8) -> Result<(), VirtioBlkError> {
        if requested == 0 { self.reset(); return Ok(()); }
        if requested & !VIRTIO_STATUS_KNOWN != 0 || requested & self.status != self.status { return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested }); }
        if requested & VIRTIO_STATUS_DRIVER != 0 && requested & VIRTIO_STATUS_ACKNOWLEDGE == 0 { return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested }); }
        if requested & VIRTIO_STATUS_FEATURES_OK != 0 {
            if requested & (VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER) != (VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER) { return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested }); }
            if self.driver_features != VIRTIO_F_VERSION_1 { self.status = requested & !VIRTIO_STATUS_FEATURES_OK; return Ok(()); }
        }
        if requested & VIRTIO_STATUS_DRIVER_OK != 0 && (requested & VIRTIO_STATUS_FEATURES_OK == 0 || !self.queue_enabled) { return Err(VirtioBlkError::InvalidDeviceStatus { current: self.status, requested }); }
    ...
    fn write_queue_size(&mut self, size: u16) -> Result<(), VirtioBlkError> {
        ...
        if size == 0 || size > VIRTIO_QUEUE_MAX_SIZE || !size.is_power_of_two() { return Err(VirtioBlkError::InvalidQueueSize { size, maximum: VIRTIO_QUEUE_MAX_SIZE }); }
    ...
    fn write_queue_enable(&mut self, value: u16) -> Result<(), VirtioBlkError> {
        ...
        match value {
            ...
            1 if self.queue_desc != 0 && self.queue_driver != 0 && self.queue_device != 0 => self.queue_enabled = true,
    ...
    fn ensure_queue_ready(&self) -> Result<(), VirtioBlkError> {
        if self.status & VIRTIO_STATUS_DRIVER_OK == 0 || self.status & VIRTIO_STATUS_FEATURES_OK == 0 || self.driver_features != VIRTIO_F_VERSION_1 || !self.queue_enabled || self.queue_desc == 0 || self.queue_driver == 0 || self.queue_device == 0 { Err(VirtioBlkError::QueueNotReady) } else { Ok(()) }
    }
...
fn read_u16(offset: u64, payload: &[u8]) -> Result<u16, VirtioBlkError> {
    let bytes: [u8; 2] = payload.try_into().map_err(|_| VirtioBlkError::InvalidRegisterPayload { offset, expected: 2, actual: payload.len() })?;
    Ok(u16::from_le_bytes(bytes))
}

fn read_u32(offset: u64, payload: &[u8]) -> Result<u32, VirtioBlkError> {
    let bytes: [u8; 4] = payload.try_into().map_err(|_| VirtioBlkError::InvalidRegisterPayload { offset, expected: 4, actual: payload.len() })?;
...
fn checked_add(base: u64, offset: u64) -> Result<u64, VirtioBlkError> {
    base.checked_add(offset).ok_or(VirtioBlkError::AddressOverflow { base, offset })
}
...
mod tests {
    ...
    fn pci_function_exposes_modern_blk_identity_and_device_config_capability() {
        ...
        assert_eq!(function.read_dword(0x00), (u32::from(VIRTIO_BLK_PCI_DEVICE_ID) << 16) | u32::from(VIRTIO_PCI_VENDOR_ID));
    ...
    fn capacity_register_is_one_sector_and_sector_payload_is_stable() {
        ...
        assert_eq!(device.read(VIRTIO_BLK_CONFIG_OFFSET, 8).unwrap(), 1_u64.to_le_bytes());
    ...
    fn three_descriptor_read_request_fills_data_status_and_used_ring() {
        ...
        device.status = VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER | VIRTIO_STATUS_FEATURES_OK | VIRTIO_STATUS_DRIVER_OK;
        device.queue_size = 4;
        ...
        write_descriptor(&mut memory, DESC, 1, DATA, VIRTIO_BLK_SECTOR_SIZE as u32, VIRTQ_DESC_F_NEXT | VIRTQ_DESC_F_WRITE, 2);
        ...
        memory.write(GuestPhysAddr::new(AVAIL + 2), &1_u16.to_le_bytes()).unwrap();
        memory.write(GuestPhysAddr::new(AVAIL + 4), &0_u16.to_le_bytes()).unwrap();
    ...
    fn write_descriptor(memory: &mut GuestMemory, table: u64, index: u16, address: u64, length: u32, flags: u16, next: u16) {
        ...
        memory.write(GuestPhysAddr::new(table + 16 * u64::from(index)), &descriptor).unwrap();
    }
}
