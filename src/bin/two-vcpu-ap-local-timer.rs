use mini_hypervisor::portio::two_vcpu_ap_local_timer_fixture::{
    run_two_vcpu_ap_local_timer, AP_LOCAL_TIMER_DIVIDE_CONFIGURATION,
    AP_LOCAL_TIMER_INITIAL_COUNT, AP_LOCAL_TIMER_VECTOR,
};
...
fn main() -> ExitCode {
    match run_two_vcpu_ap_local_timer() {
        Ok(result) => {
            ...
            println!("AP local timer initial MP state: {}", result.initial_ap_mp_state());
            println!(
