...
fn main() -> ExitCode {
    match run_versioned_page_vcpu_checkpoint_guest(VmConfig::default()) {
        Ok(result) => {
            ...
            println!(
                "versioned checkpoint schema: version={} encoded_bytes={} msrs={}",
                result.schema_version(),
                result.encoded_len(),
                result.msr_count()
            );
            println!(
                "versioned checkpoint pages: {:#x},{:#x},{:#x}",
                MULTI_PAGE_CHECKPOINT_CONTROL_PAGE.get(),
                MULTI_PAGE_CHECKPOINT_DATA_PAGE.get(),
                MULTI_PAGE_CHECKPOINT_STACK_PAGE.get()
            );
            println!(
                "versioned checkpoint corruption: control={} data={} stack={} vcpu={}",
                checkpoint
                    .corruption()
                ...
                checkpoint
                    ...
                    .unwrap_or(false),
                checkpoint.corruption().vcpu().is_exact_match()
            );
            println!(
                "versioned checkpoint restore: control={} data={} stack={} vcpu={}",
                checkpoint
                    .restored()
                    .page_exact(MULTI_PAGE_CHECKPOINT_CONTROL_PAGE)
                    .unwrap_or(false),
                ...
                checkpoint
                    ...
                    .page_exact(MULTI_PAGE_CHECKPOINT_STACK_PAGE)
                    .unwrap_or(false),
                checkpoint.restored().vcpu().is_exact_match()
            );
            println!("versioned checkpoint proof: {:?}", checkpoint.proof());
            println!(
                "versioned checkpoint capture: {}",
                checkpoint.checkpoint_report()
            );
            println!(
                "versioned checkpoint terminal: {}",
