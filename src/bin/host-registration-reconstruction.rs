...
fn main() -> ExitCode {
    match run_full_controller_virtio_blk_host_registration_reconstruction_guest() {
        Ok(result) => {
            ...
            println!(
                "host-registration checkpoint page: {:#x}",
                FULL_CONTROLLER_VIRTIO_BLK_CHECKPOINT_PAGE.get()
            );
            println!(
                "host-registration descriptor: doorbell={:#x} len={} datamatch={:#x} gsi={}",
                result.doorbell_gpa(),
                result.doorbell_length(),
                result.doorbell_datamatch(),
                result.gsi()
            );
            println!(
                "host-registration capture: rip={:#x} rflags={:#x}",
                result.capture_rip(),
                result.capture_rflags()
            );
            println!(
                "host-registration captured queue: avail={} used={}",
            ...
            println!(
                "host-registration mutation transport: doorbell-events={} irqfd-signals={}",
                result.mutation_doorbell_events(),
                result.mutation_irqfd_signals()
            );
            println!(
                "host-registration mutation proof: {:?}",
            ...
            println!(
                "host-registration replay transport: doorbell-events={} irqfd-signals={}",
                result.replay_doorbell_events(),
                result.replay_irqfd_signals()
            );
            println!(
                "host-registration replay queue: avail={} used={}",
                result.replay_avail_idx(),
                result.replay_used_idx()
            );
            println!(
                "host-registration replay proof: {:?}",
                result.replay_proof()
            );
            println!(
                "host-registration replay rflags: {:#x}",
                result.replay_rflags()
            );
            println!(
                "host-registration backing/readback match: {}",
