...
- validate a long-mode boot layout requiring guest RAM at GPA 0 with at least 2 MiB, an entry in either the preserved identity map or a validated alias page, and a non-zero low identity-mapped stack pointer outside bootstrap page-table pages;
- construct the long-mode PML4 at `0x1000`, PDPT at `0x2000`, PD at `0x3000`, and bounded alias PT at `0x4000`; preserve one 2 MiB identity-mapped large page covering virtual/physical `0..0x20_0000`, and link only validated 4 KiB alias PTEs when a mapped layout requests them;
...
- service exactly one byte-wide MMIO device at fixture GPA `0x2000` through `MmioBus`, recording writes and returning one configured read byte;
...
- run pure validation/UAPI/CPUID/MSR/state-snapshot/flat-loader/ELF64-loader/long-mode/exit-dispatch/MMIO/KVM-unknown/exception/fail-entry/internal-error/system-event/port-bus/execution-budget tests without requiring KVM;
- run environment-sensitive KVM integration tests, plus strict CI long-mode, ELF64, and MMIO proofs that require usable `/dev/kvm` and fail rather than skipping when any milestone fixture cannot execute.
...
This remains a single-vCPU x86 execution laboratory. It provides one deterministic x86-64 long-mode bootstrap, a preserved low 2 MiB identity map, one fixed 2 MiB ELF alias virtual window backed by validated low-RAM pages, one bounded ELF64 `ET_EXEC` loader/execution path, and one fixed byte-wide userspace MMIO device proof, but it does **not** provide a general virtual-memory manager, dynamic page-table allocator, page-permission/NX policy, ELF relocations, `ET_DYN`/PIE, dynamic linking, a general executable address-layout policy, an MMIO range/device registry, virtual MMIO mapping, PCI, interrupts, an in-kernel interrupt controller model, arbitrary/configurable CPU models, virtio, SMP, Linux boot, migration orchestration, whole-VM snapshots, guest-memory/device snapshots, resumable execution, architectural rollback, exception injection/recovery policy, KVM-unknown hardware-reason recovery policy, fail-entry retry/placement policy, internal-error recovery/retry policy, instruction emulation, or implemented system-event lifecycle policy.
...
Most environment-sensitive integration tests continue to distinguish unavailable `/dev/kvm` from product regressions. The strict CI gates directly run `run-long-mode`, `run-elf64`, and `run-mmio`; those gates require a usable KVM device and must observe the exact `LM64`, `LM64`, and `RMIO` proofs plus each fixture's exact terminal HLT RIP and architectural RFLAGS bit 1. KVM unavailability is not milestone success.
