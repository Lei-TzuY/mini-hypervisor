...
The candidate derivation is pure Rust over already-validated owned host state; it issues no KVM call and grants no new authority. `HostMsrModelCandidate` remains a host-only compatibility-analysis input. Its immutable-only content is not a migration guarantee, does not authorize guest MSR access, and is not copied into `Vm` or `Vcpu`. This milestone issues no additional MSR ioctl, no vCPU `KVM_GET_MSRS`, no `KVM_SET_MSRS`, reads or writes no guest MSR value, and derives no guest MSR allow/deny/value policy.
