...
There is no guest virtual-address translation, external guest-image parser, MMIO, interrupt injection, configurable/migratable CPU model, combined CPU-model candidate, guest MSR policy or guest MSR value lifecycle, virtqueue, disk backend, snapshot decoder, dynamic device registration, scheduler, or guest-controlled device descriptor parsing in this revision. The backend has bounded typed general MSR-index, feature-index, and stability-annotated feature-value capability snapshots; callers can derive provenance-bound immutable-only `HostMsrModelCandidate` values and compare them with `HostMsrModelComparison`. Configured `GuestCpuPolicy` values can now be compared separately with `GuestCpuPolicyComparison`. None of these analyses confers guest MSR semantics or migration stability. The current CPUID contract is deliberately host-derived and conservatively masked rather than a stable cross-host CPU profile. Unsupported exits and unsupported port requests are rejected with structured diagnostics rather than serviced heuristically.
