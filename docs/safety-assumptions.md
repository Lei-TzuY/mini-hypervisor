...
The resulting stability-annotated `HostMsrFeatureValues` remains host capability discovery state. It is retained by `KvmBackend`, is not copied into `Vm` or `Vcpu`, does not authorize guest MSR access, and does not become a migration-stable CPU model merely because a value is `ModelImmutable`. This milestone issues no additional MSR ioctl, no vCPU `KVM_GET_MSRS`, no `KVM_SET_MSRS`, reads or writes no guest MSR value, and derives no guest MSR allow/deny/value policy.
