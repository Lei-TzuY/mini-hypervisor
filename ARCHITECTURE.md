...
For `KVM_EXIT_INTERNAL_ERROR`, `Vcpu::internal_error` is the only layer allowed to form the tested x86 base internal-error view. The union payload begins at offset 32, but the current typed view ends immediately after the always-available `suberror: u32`, giving a 40-byte required prefix after struct alignment. Extraction first requires the current exit reason to be internal error, then copies only `suberror` into owned `VcpuInternalError` state. It deliberately does not read, validate, slice, copy, or expose the capability-dependent `ndata` or `data[16]` fields, so this boundary neither requires nor implies `KVM_CAP_INTERNAL_ERROR_DATA`. No raw pointer or borrowed internal-error payload crosses into dispatch.
...
`KVM_EXIT_INTERNAL_ERROR` now has typed classification and an owned base diagnostic containing only the always-available raw `suberror`. The current boundary deliberately does not query or require `KVM_CAP_INTERNAL_ERROR_DATA`, does not read capability-dependent `ndata` or `data[16]`, and does not interpret `suberror` into emulation recovery, retry, replacement execution, or lifecycle policy. Any optional-data support or recovery semantics require a separate explicitly bounded design.
