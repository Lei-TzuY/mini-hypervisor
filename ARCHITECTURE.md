...
The general-index, feature-index, and feature-value snapshots remain owned only by `KvmBackend` and are exposed read-only. Model candidates and comparisons are materialized on demand from owned safe Rust state rather than duplicated as backend state. None is copied into `Vm` or `Vcpu`, and no guest MSR allow/deny/value policy is derived from the feature snapshot. The system-level feature query remains host capability discovery; vCPU architectural-state readback is a separate caller-directed boundary described below.
...
This is a read-only primitive, not a guest MSR policy or restore lifecycle. It provides no `KVM_SET_MSRS`, does not authorize any index for writing, does not claim a returned value is migration-stable, and does not automatically add vCPU state to `CpuModelCandidate` or `CpuModelComparison`.
...
The next bounded slice should define an explicit, pure guest-MSR access-policy boundary before any write/restore primitive is introduced. It should take a caller-selected `MsrIndex` set and a validated `HostMsrIndexList`, reject indices not present in the general KVM-supported set, preserve deterministic caller order while defining an explicit duplicate rule, and return an owned read/write-authority description without issuing any ioctl or attaching MSR values. The policy must not be derived from `HostMsrFeatureValues`, `HostMsrModelCandidate`, or `VcpuMsrValues`; those are observations with different semantics. This slice must not add `KVM_SET_MSRS`, mutate vCPU state, claim policy membership is migration-safe, define a named migration-stable CPU model, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.