      ...
      Vm
       ...
       └─ vCPU creation
             ...
             Vcpu
              ...
              ├─ checked KVM_EXIT_INTERNAL_ERROR base-suberror extraction
...
Separately from that required set, `KvmBackend` queries `KVM_CAP_INTERNAL_ERROR_DATA` (capability ID 40) through the same `KVM_CHECK_EXTENSION` boundary and stores the raw returned value in the existing owned `HostCapabilities.extensions` snapshot. A missing observation in manually constructed `HostCapabilities` or a recorded value of `0` does not invalidate an otherwise valid host; `internal_error_data_capability()` exposes the recorded observation when present and `supports_internal_error_data()` is true exactly when its raw value is greater than zero. This is host metadata only: a positive observation does not expand the current `kvm_run` internal-error view or authorize decoding capability-dependent payload fields.
...
`Vcpu::run_once` retries an interrupted host syscall, performs one completed `KVM_RUN`, reads the tested x86 prefix of `kvm_run`, and returns a typed `VcpuExit`. HLT, port I/O, legacy `KVM_EXIT_SHUTDOWN`, `KVM_EXIT_FAIL_ENTRY`, base `KVM_EXIT_INTERNAL_ERROR`, and `KVM_EXIT_SYSTEM_EVENT` are classified explicitly; unknown reasons retain the exact raw reason.
...
For `KVM_EXIT_INTERNAL_ERROR`, `Vcpu::internal_error` is the only layer allowed to form the tested x86 base internal-error view. The union payload begins at offset 32, but the current typed view ends immediately after the always-available `suberror: u32`, giving a 40-byte required prefix after struct alignment. Extraction first requires the current exit reason to be internal error, then copies only `suberror` into owned `VcpuInternalError` state. It deliberately does not read, validate, slice, copy, or expose the capability-dependent `ndata` or `data[16]` fields. The backend separately records the raw `KVM_CAP_INTERNAL_ERROR_DATA` observation, but this decoder does not consult that value and does not require the capability; even a positive observation does not expand the 40-byte typed view. No raw pointer or borrowed internal-error payload crosses into dispatch.
...
Unhandled raw exits, fail-entry diagnostics, internal-error diagnostics, and system-event diagnostics are also annotated with the complete ordered trace accumulated by the loop. A fail-entry reason `9`, internal-error reason `17`, or system-event reason `24` appears exactly once at the tail because the successful `KVM_RUN` is recorded before payload decoding/dispatch. This diagnostic ownership does not create resumable execution or partial-success result semantics.
...
- `KVM_EXIT_INTERNAL_ERROR` is decoded into owned base `suberror` diagnostics and returned as structured `VmExitError::InternalError` without issuing `KVM_GET_REGS` or another secondary vCPU ioctl; optional internal-error `ndata/data` and recovery policy are deliberately outside this boundary.
...
`KVM_EXIT_INTERNAL_ERROR` now has typed classification and an owned base diagnostic containing only the always-available raw `suberror`. The backend separately queries `KVM_CAP_INTERNAL_ERROR_DATA` and retains its raw observation, but that capability is not required and the base decoder deliberately does not use the observation to read capability-dependent `ndata` or `data[16]`. A positive observation therefore does not imply optional-data support, emulation recovery, retry, replacement execution, or lifecycle policy; those semantics require a separate explicitly bounded design.
