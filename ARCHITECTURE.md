...
`Vcpu::capture_msr_snapshot` builds on that path rather than introducing another read contract. After a complete policy-bound capture succeeds, it validates and owns the exact policy/value pairing as `GuestMsrSnapshot`. Readback, value-set materialization, and full-snapshot certification therefore remain separate typed boundaries.

Readback, capture, snapshot certification, and write remain deliberately separate primitives. `Vcpu::msrs` is caller-index-directed, `Vcpu::capture_msrs` is full-policy-directed, `Vcpu::capture_msr_snapshot` additionally binds the complete policy to the captured values, and `Vcpu::set_msrs` accepts only an already policy-validated `GuestMsrValueSet`. None automatically adds vCPU state to `CpuModelCandidate` or `CpuModelComparison`, automatically restores another vCPU, or makes a migration-stability claim.
...
The next bounded slice should add a snapshot-bound vCPU MSR restore attempt that accepts an existing `GuestMsrSnapshot` and delegates exactly once to the existing `Vcpu::set_msrs(snapshot.values())` path. It must not expose an arbitrary value-set bypass, revalidate or reinterpret the snapshot's policy/value coverage, retry a short `KVM_SET_MSRS`, roll back a successfully written prefix, or claim transactional restore. Exact completion may be reported as success; partial completion must preserve the existing `VcpuMsrPartialWrite` semantics, including the fact that the processed prefix may already have mutated the target vCPU. Focused tests should lock empty snapshot behavior, exact delegation/value order, propagation of structured partial-write diagnostics, and ownership after the original capture inputs are dropped. This slice must not automatically read back for verification, restore general registers/CPUID/device state, orchestrate multi-vCPU restore, claim migration safety, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
