...
Restore inherits the same non-transactional completion semantics as `set_msrs`. A short write propagates `VcpuMsrPartialWrite` unchanged, including the warning that the processed prefix may already have mutated the target vCPU. The restore boundary does not retry, roll back, or automatically recapture state for verification.
...
`Vcpu::capture_msr_snapshot` builds on that path rather than introducing another read contract. After a complete policy-bound capture succeeds, it validates and owns the exact policy/value pairing as `GuestMsrSnapshot`. Readback, value-set materialization, full-snapshot certification, pure snapshot comparison, direct write, and snapshot-bound restore therefore remain separate typed boundaries.
...
`Vcpu::msrs`, `Vcpu::capture_msrs`, `Vcpu::capture_msr_snapshot`, `Vcpu::set_msrs`, and `Vcpu::restore_msr_snapshot` are synchronous state operations over the same owned vCPU descriptor used by the execution path. The current VMM has no scheduler or concurrent vCPU runner; callers invoke these primitives directly rather than racing an independently executing vCPU thread. Readback returns owned typed values, policy-bound capture returns an owned validated value set, full snapshot capture returns an owned policy/value binding, direct writes consume only an owned policy-validated value set, and snapshot restore delegates the snapshot's owned value set to the same write boundary. None retains a caller borrow after its operation returns.
...
The next bounded slice should add snapshot-bound restore verification without changing the existing write semantics. It should accept one `GuestMsrSnapshot`, delegate exactly once to the existing `Vcpu::restore_msr_snapshot`, and only after exact write completion recapture the same policy through `Vcpu::capture_msr_snapshot(snapshot.policy())`. The expected snapshot and recaptured snapshot should then be compared through the existing pure `GuestMsrSnapshot::compare` contract and returned as an owned verification report. A write failure, including `VcpuMsrPartialWrite`, must propagate unchanged and must not trigger readback; a readback/capture failure after a successful write must propagate without retry or rollback. An exact snapshot comparison means only that the recaptured MSR state equals the expected snapshot under this project contract; a mismatch must remain an explicit report rather than triggering automatic repair. Focused tests should lock empty behavior, write-before-read sequencing, no read after write failure, readback failure propagation, exact verification, and mismatch reporting. This slice must not restore general registers/CPUID/device state, orchestrate multi-vCPU restore, retry or roll back partial mutation, claim transactional restore or migration safety, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
