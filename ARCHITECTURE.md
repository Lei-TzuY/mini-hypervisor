...
The next bounded slice should add a pure owned `CpuModelCandidate` composition boundary that pairs one configured `GuestCpuPolicy` with one immutable `HostMsrModelCandidate`. Construction should clone/own both existing typed inputs without re-deriving either contract, retain the MSR candidate's complete source-observation provenance, and expose the two components read-only. It must not collapse the existing CPUID and MSR comparison semantics into a migration verdict, perform any KVM operation, change CPUID policy construction or application, derive/apply guest MSR policy, call vCPU `KVM_GET_MSRS`/`KVM_SET_MSRS`, define a named migration-stable CPU model, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
