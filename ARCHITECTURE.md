...
The next bounded slice should add a typed, bounded vCPU MSR readback primitive over an explicit caller-supplied `MsrIndex` set. It should reuse the existing fixed `KvmMsrs<N>` ABI machinery, reject a request that exceeds the project bound before issuing `KVM_GET_MSRS`, preserve caller index order, require the ioctl completion count to equal the requested count, and require every returned entry index to match the requested index at the same position before returning owned typed values. An empty request should return an empty result without issuing an ioctl. This slice must not automatically derive the requested set from `HostMsrFeatureValues` or `HostMsrModelCandidate`, treat system-level feature-MSR observations as guest vCPU state, add `KVM_SET_MSRS`, define a guest MSR allow/deny/value policy, define a named migration-stable CPU model, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
