...
The raw ioctl UAPI details live in `src/kvm/sys.rs`; the tested `KVM_EXIT_UNKNOWN`, `KVM_EXIT_FAIL_ENTRY`, capability-gated `KVM_EXIT_INTERNAL_ERROR`, and `KVM_EXIT_SYSTEM_EVENT` `kvm_run` payload views are isolated in `src/vcpu/kvm_unknown.rs`, `src/vcpu/fail_entry.rs`, `src/vcpu/internal_error.rs`, and `src/vcpu/system_event.rs`. The internal-error module also owns the pure `VcpuInternalErrorSuberror` classification layered on the copied raw `suberror`; higher layers call typed Rust methods and do not issue raw `ioctl` operations or inspect raw shared-memory payload layouts directly.
...
`Vcpu::run_once` retries an interrupted host syscall, performs one completed `KVM_RUN`, reads the tested x86 prefix of `kvm_run`, and returns a typed `VcpuExit`. HLT, port I/O, legacy `KVM_EXIT_SHUTDOWN`, `KVM_EXIT_UNKNOWN`, `KVM_EXIT_FAIL_ENTRY`, `KVM_EXIT_INTERNAL_ERROR`, and `KVM_EXIT_SYSTEM_EVENT` are classified explicitly; other unsupported reasons retain the exact raw reason.
...
For `KVM_EXIT_UNKNOWN`, `Vcpu::kvm_unknown_exit` is the only layer allowed to inspect the tested fixed x86 unknown-exit union member. It first requires the current raw exit reason to be `0`, then copies KVM's `hardware_exit_reason` into owned `VcpuKvmUnknownExit` state. Calling this accessor for another exit reason returns structured `KvmUnknownPayloadUnavailable` instead of reading the wrong union member. The hardware reason remains opaque diagnostic metadata: no SGX/VMX interpretation, retry, recovery, replacement execution, or lifecycle action is inferred, and no raw pointer or borrowed payload crosses into dispatch.
...
For `KVM_EXIT_INTERNAL_ERROR`, `Vcpu::internal_error` is the only layer allowed to inspect the tested x86 internal-error union. The always-available base view begins at union offset 32 and ends after `suberror: u32`; when the inherited optional-support flag is false, decoding forms only that base view, copies raw `suberror`, and exposes `VcpuInternalError::data()` as `None`. The owned scalar remains available through `suberror()`, while `suberror_kind()` derives the policy-neutral `VcpuInternalErrorSuberror` classification: raw Linux KVM values 1, 2, 3, and 4 map to `Emulation`, `SimultaneousExceptions`, `DeliveryEvent`, and `UnexpectedExitReason`; every other value is retained exactly as `Unknown(raw)` and round-trips through `raw()`. When the vCPU inherited a positive `KVM_CAP_INTERNAL_ERROR_DATA` observation, decoding may instead form the fixed full 168-byte prefix containing `suberror`, `ndata`, and `data[16]`; classification is derived from the same copied raw suberror and therefore behaves identically on both decoder paths. Kernel `ndata` is untrusted and must be `<= 16` before any slice is formed; only the declared words are copied into owned `VcpuInternalError` state. `ndata == 0` is represented as available-but-empty data (`Some(&[])`), which is distinct from capability absence. An out-of-capacity count becomes structured `InvalidInternalErrorDataCount` diagnostics retaining vCPU id, raw `suberror`, reported `ndata`, capacity, and later the execution trace. No raw pointer or borrowed internal-error payload crosses into dispatch, and neither the typed suberror nor any optional payload value is interpreted as recovery policy.
...
Unhandled raw exits, KVM-unknown diagnostics, fail-entry diagnostics, internal-error diagnostics (including invalid optional-data counts), and system-event diagnostics are also annotated with the complete ordered trace accumulated by the loop. A KVM-unknown reason `0`, fail-entry reason `9`, internal-error reason `17`, or system-event reason `24` appears exactly once at the tail because the successful `KVM_RUN` is recorded before payload decoding/dispatch. This diagnostic ownership does not create resumable execution or partial-success result semantics.
...
- `KVM_EXIT_UNKNOWN` is decoded into owned `hardware_exit_reason` diagnostics and returned as structured `VmExitError::KvmUnknownExit` without issuing `KVM_GET_REGS` or another secondary vCPU ioctl that could obscure the purpose-built hardware diagnostic. Misuse of the payload accessor is represented separately as `VmExitError::KvmUnknownPayloadUnavailable`.
...
- `KVM_EXIT_INTERNAL_ERROR` is decoded into owned raw `suberror` plus capability-gated optional data and returned as structured `VmExitError::InternalError` without issuing `KVM_GET_REGS` or another secondary vCPU ioctl; malformed capability-enabled `ndata` becomes `VmExitError::InvalidInternalErrorDataCount`. The structured errors intentionally retain the raw scalar rather than replacing it with the enum, so callers can apply the same lossless `VcpuInternalErrorSuberror::from_raw` classification without changing error shape. Neither path adds suberror/data interpretation, recovery, retry, replacement execution, or lifecycle policy.
...
- Other unsupported raw exit reasons become `VmExitError::Unhandled` with vCPU id and register diagnostics.
...
The dispatcher deliberately does **not** snapshot registers for an in-flight KVM I/O exit. KVM defines port-I/O operations as pending until userspace re-enters `KVM_RUN`; register state used as a completed-operation diagnostic is therefore taken only on a later terminal exit. KVM-unknown, fail-entry, and internal-error diagnostics likewise deliberately avoid a secondary register read: the purpose-built payload from the completed exit is preserved directly rather than risking replacement by a subsequent ioctl failure.
...
`KVM_EXIT_UNKNOWN` now has a distinct typed path and owned `hardware_exit_reason` diagnostic, but the hardware reason is deliberately opaque. The VMM does not infer SGX/VMX meaning, retry, recovery, replacement execution, or lifecycle behavior from it; generic unsupported raw exits remain separate `Unhandled { reason }` diagnostics.
...
`KVM_EXIT_INTERNAL_ERROR` now has typed exit classification, lossless typed suberror classification, and capability-gated optional payload diagnostics. The always-available raw `suberror` is copied on every internal-error exit and remains the source of truth; known Linux KVM values 1 through 4 may be viewed as `Emulation`, `SimultaneousExceptions`, `DeliveryEvent`, and `UnexpectedExitReason`, while unknown raw values are preserved. The backend separately queries optional `KVM_CAP_INTERNAL_ERROR_DATA` and propagates only positive support into created vCPUs; hosts without support remain valid and use the base-only path. Capability-enabled decoding validates `ndata <= 16` before slicing and owns only the declared `data[16]` prefix, with empty declared data kept distinct from unavailable data. Neither the typed suberror nor the richer payload boundary creates suberror/data-specific emulation recovery, retry, replacement execution, or lifecycle policy.
...
`KVM_EXIT_EXCEPTION` raw reason `1` is a distinct typed `VcpuExit::Exception` path. `Vcpu::exception_exit()` is the only layer allowed to inspect its fixed x86 `kvm_run` union member: the view begins at union offset 32, contains only `exception: u32` and `error_code: u32`, has an 8-byte payload and a 40-byte required prefix, and therefore does not increase the existing 168-byte common mapping-size floor. Calling the accessor for any other current exit returns structured `ExceptionPayloadUnavailable` rather than interpreting the wrong union member.

A successful decode copies both fields into owned `VcpuException` state before the values cross into VM-exit policy. The central dispatcher returns structured `VmExitError::Exception` directly from that purpose-built payload and deliberately issues no `KVM_GET_REGS` or other secondary vCPU ioctl that could obscure the completed exception exit. The execution loop records reason `1` before dispatch and replaces the local diagnostic trace with the complete ordered completed-exit trace, so the exception reason appears exactly once at the trace tail.

Exception vector and error-code values remain opaque diagnostic metadata. This boundary does not inject or reinject exceptions, emulate instructions, retry or replace execution, recover the guest, mutate VM lifecycle state, or infer architecture-specific policy from either field.
