...
The next bounded slice should add an owned full-policy MSR snapshot type that binds one complete `GuestMsrAccessPolicy` to the exact `GuestMsrValueSet` produced by a successful policy-bound capture. Construction should be crate-controlled so callers cannot label an arbitrary strict subset as a full snapshot. The snapshot must own both components, preserve policy/value order, require one value for every policy entry with matching indices, and expose both read-only after the source policy and capture result are dropped. It should be pure Rust and should not issue `KVM_GET_MSRS` or `KVM_SET_MSRS` itself. Focused tests should lock empty snapshots, full coverage, index/order equality, ownership, and rejection of missing/extra/reordered values. This slice must not automatically restore another vCPU, retry or roll back partial writes, combine general registers/CPUID/device state, claim migration safety, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
