...
`CpuModelCandidate` deliberately has no combined compatibility or migration verdict. It is only a typed analysis unit that keeps the configured CPUID policy and immutable host-MSR candidate together without weakening either component's existing semantics.
...
The next bounded slice should add a pure owned `CpuModelComparison` report that compares two `CpuModelCandidate` values by delegating to the already-defined `GuestCpuPolicy::compare` and `HostMsrModelCandidate::compare` contracts, then retains both component comparison results together. It should preserve the independent CPUID and MSR directional diagnostics and provenance without inventing a stronger compatibility rule or flattening them into one migration-safe boolean. It must not perform any KVM operation, change CPUID/MSR derivation or comparison semantics, derive/apply guest MSR policy, call vCPU `KVM_GET_MSRS`/`KVM_SET_MSRS`, define a named migration-stable CPU model, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.
