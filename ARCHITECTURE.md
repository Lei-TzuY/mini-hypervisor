...
Successful results own only typed `GuestMsrValue` records containing `MsrIndex` plus `u64`. They keep no borrow into the policy or caller slice and carry no `MsrFeatureStability`, host feature observation, model-candidate provenance, or vCPU descriptor state. Authorization and value-set validation still do not prove that KVM will accept a particular future write value; per-MSR architectural constraints and `KVM_SET_MSRS` completion semantics remain a later boundary. The value set is not a migration-safety statement.
...
This is a read-only primitive, not a restore lifecycle. It provides no `KVM_SET_MSRS`, does not consult `GuestMsrAccessPolicy` automatically, does not claim a returned value is migration-stable, and does not automatically add vCPU state to `CpuModelCandidate` or `CpuModelComparison`.
...
`Vcpu::msrs` is a synchronous vCPU-state query over the same owned vCPU descriptor used by the execution path. The current VMM has no scheduler or concurrent vCPU runner; callers invoke the readback primitive directly rather than through the VM-exit loop. It returns only owned typed values after the request/response checks described above.
...
The next bounded slice should define and implement a bounded vCPU `KVM_SET_MSRS` write boundary that accepts only an already-validated `GuestMsrValueSet`. Focused pure tests should first lock exact request encoding, the 1024-entry project bound, zero reserved fields, caller value-set order, empty-request behavior, and completion-count validation. The minimum write primitive should skip the ioctl for an empty set, use the fixed bounded KVM MSR buffer, and treat KVM's processed-entry return as non-transactional: a short return means the successful prefix may already have mutated vCPU state, so the error must report the processed count and first unwritten index without retrying, rolling back, or claiming atomic failure. A returned count greater than requested must be rejected as invalid response metadata. This slice must not accept arbitrary `(MsrIndex, u64)` input directly, silently bypass `GuestMsrAccessPolicy`, derive values from host feature/model observations, perform multi-step restore orchestration, claim arbitrary values are valid merely because their indices are authorized, claim migration safety, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.