...
There is no configurable or migration-stable CPU model yet. The current boundary distinguishes discovered host support from a derived guest CPUID policy, but there is still exactly one built-in CPUID policy: host/KVM-supported entries with conservative masking for the absent LAPIC model. The vCPU creation path requires KVM to report back exactly that submitted typed CPUID policy, and the existing CPUID fixture independently verifies selected architectural bits from guest-observed state. `HostMsrIndexList`, `HostMsrFeatureIndexList`, stability-annotated `HostMsrFeatureValues`, `HostMsrModelCandidate`, and `HostMsrModelComparison` remain host-only state; neither `ModelImmutable`, candidate membership, nor an exact candidate comparison is promoted into a cross-host migration guarantee, and there is still no guest MSR policy or guest MSR value lifecycle.
...
The next bounded slice should add a pure compatibility/diff contract for the actual `GuestCpuPolicy` CPUID contract, keyed by the KVM CPUID identity tuple `(function, index)` rather than list position. It should distinguish missing policy entries, extra policy entries, and same-key differences across flags/EAX/EBX/ECX/EDX while preserving both complete policies in the comparison result. Reordered but otherwise identical policy entries should compare as the same guest CPU contract. This should remain pure Rust and must not change policy construction, CPUID masking, `KVM_SET_CPUID2`, read-back verification, guest execution, MSR behavior, or claim that matching CPUID and MSR analyses alone establish migration safety.
