...
Pure guest-MSR policy construction has its own `GuestMsrPolicyError`, because unsupported/duplicate caller authorization is configuration validation and does not originate from a host I/O or vCPU operation. Future MMIO, interrupt, snapshot, and stronger invariant categories will be added only when those responsibilities exist.
...
The next bounded slice should add a pure, owned guest-MSR value-set type that validates caller-supplied `(MsrIndex, u64)` state against an existing `GuestMsrAccessPolicy` before any write ioctl exists. It should preserve caller order, reject duplicate value indices, reject every index not authorized by the policy, define whether the value set must cover all policy entries or may be a validated subset, and return no partial state on failure. The value set must carry no host feature-stability metadata, issue no ioctl, and make no claim that KVM will accept every value merely because its index is authorized. This slice must not add `KVM_SET_MSRS`, mutate vCPU state, perform restore orchestration, derive values from `HostMsrFeatureValues` or `HostMsrModelCandidate`, claim migration safety, define a named migration-stable CPU model, or add long-mode boot, interrupts, MMIO, SMP, or device expansion in the same slice.