...
The raw ioctl UAPI details live in `src/kvm/sys.rs`; the tested `KVM_EXIT_FAIL_ENTRY` and `KVM_EXIT_SYSTEM_EVENT` `kvm_run` payload views are isolated in `src/vcpu/fail_entry.rs` and `src/vcpu/system_event.rs`. Higher layers call typed Rust methods and do not issue raw `ioctl` operations or inspect raw shared-memory payload layouts directly.
...
`Vcpu::run_once` retries an interrupted host syscall, performs one completed `KVM_RUN`, reads the tested x86 prefix of `kvm_run`, and returns a typed `VcpuExit`. HLT, port I/O, legacy `KVM_EXIT_SHUTDOWN`, `KVM_EXIT_FAIL_ENTRY`, and `KVM_EXIT_SYSTEM_EVENT` are classified explicitly; unknown reasons retain the exact raw reason.
...
The dispatcher deliberately does **not** snapshot registers for an in-flight KVM I/O exit. KVM defines port-I/O operations as pending until userspace re-enters `KVM_RUN`; register state used as a completed-operation diagnostic is therefore taken only on a later terminal exit. Fail-entry diagnostics likewise deliberately avoid a secondary register read: the purpose-built payload from the completed exit is preserved directly rather than risking replacement by a subsequent ioctl failure.
